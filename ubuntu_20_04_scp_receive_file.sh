...
if [ -z "$SSH_CONFIGURATION" ]; then
  ...
  read -p "Please enter the path where the file should be copied: " LOCAL_FILEPATH                                                                                  # Prompt user to enter local filepath
  ...
  if [ -z "$LOCAL_FILEPATH" ]; then
    ...
    echo " "
    read -p "Please enter the exact path of the file (on the remote device) that you want to receive: " FILE_TO_RECEIVE                                             # Prompt user to enter remote filepath


    # Continuing only if user chose to continue (OK button or enter)
    if [ -z "$FILE_TO_RECEIVE" ]; then
      echo "\n$($CURRENT_DATE): The script was cancelled by the user when prompting for the file to receive.\n\n$($CURRENT_DATE): Exiting.\n"
      exit 3
    else
