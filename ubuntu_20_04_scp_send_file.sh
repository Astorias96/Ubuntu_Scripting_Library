...
if [ -z "$SSH_CONFIGURATION" ]; then
  ...
  read -p "Please enter the path of the file that you want to send: " FILE_TO_SEND                                                                           # Prompt user to enter local filepath

  # Continuing only if user chose to continue (OK button or enter)
  if [ -z "$FILE_TO_SEND" ]; then
    echo "\n$($CURRENT_DATE): The script was cancelled by the user when prompting for the file to send.\n\n$($CURRENT_DATE): Exiting.\n"
    exit 2
  else
    echo " "
